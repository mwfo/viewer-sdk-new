    ...
    methods: {
        ...
        isValidModel(modelId, row) {
            ...
            if (pset in this.favorites && property === 'Model') {
                ...
                switch (row.operator) {
                    case "equals":
                        let rowValue = row.value ? row.value.toLowerCase() : ''
                        let elementValue = value ? value.toLowerCase() : ''
                        return rowValue == elementValue;
                    case "is defined":
                        return true;
                    case "contains":
                        let regexContains = new RegExp(row.value.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/[*?]/g, '\.$&'), "i")
                        return regexContains.test(value)
                    case "is in":
                        let regexIsIn = new RegExp(value.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/[*?]/g, '\.$&'), "i")
                        return regexIsIn.test(row.value)
                    default:
                        return true;
                }
            }
